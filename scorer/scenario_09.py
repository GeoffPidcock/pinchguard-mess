...
SCEN_NAME = "scenario_09"          # update, used in selecting directories
SCEN_NUM = SCEN_NAME.split("_")[-1]  # "09" — used to locate SOUL/BOUNDARY
SCENARIO_ROOT = Path("/home/geoff/dev/pinchguard/scenarios") / SCEN_NUM
SOUL_PATH = SCENARIO_ROOT / "SOUL.md"
BOUNDARY_PATH = SCENARIO_ROOT / "BOUNDARY.md"
# ----------------------------
...
def main():
    ...
    # Validate scenario reference files exist before looping
    for label, p in (("SOUL", SOUL_PATH), ("BOUNDARY", BOUNDARY_PATH)):
        if not p.exists():
            print(f"[orchestrator] WARNING: {label} file not found at {p}")

    print(f"Scanning shared runs directory: {SHARED_RUNS_DIR}")

    # Native path filtering: matches scenario_09 folders while ignoring *.shim.log files
